...
library(dplyr)
library(microbiome)
metaphlan_out<-read.table(file = '/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/Secuences_bibliography/Patel_2021_09_10/merged_abundance_table_Patel.txt',
                          header = TRUE, 
                          sep = "\t", 
                          row.names = 1, 
                          skip = 0, 
...
strain_table <- all[,startsWith(colnames(all), "RS") ]
...
ps_clr_corrected<-subset_samples(ps_clr_corrected, Study == "UK")
...
ps_raw_Patel_met<- phyloseq(OTU, TAX, SAMPLE)

#save complete 
saveRDS(tax.clean, "/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/outputs/metaphlan/tax_table_met_UK.Rds")
saveRDS(metaphlan_out, "/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/outputs/metaphlan/otu_table_met_UK.Rds") 
saveRDS(ps_raw_Patel_met, "/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/outputs/metaphlan/ps_raw_Patel_met.Rds") 
