...
ps_genus <- tax_glom(ps_comp, taxrank = 'Genus', NArm = FALSE)
...
metadata<-metadata[,c("DmGenderSexID","Group_cutoff_4", "Age", "PHES", "Ammonia",
                      ...
                      "Sodium", "Potassium", "Timepoint", "Volunteer", "Study")]
...
Hem<- subset(metadata, select = c("Haemoglobin", "INR", "Fibrinogen", "Ammonia",
                                  "Sodium", "Potassium",))
featureList<-list()
featureList$Liver_and_Kidney<-Liver
featureList$Immunology<-Imm
featureList$Hematology_and_metabolic<-Hem
featureList$Genera<-Genera
featureList$GMMs<-GMMs
featureList$ARGs<-ARGs_rgi

metadata_all <- metadata
metadata <- metadata[, c("DmGenderSexID","Group_cutoff_4", "Age", "PHES",
                         "Timepoint", "Volunteer", "Study")]
...
metadata <- fastDummies::dummy_cols(metadata, select_columns = c("Group_cutoff_4"))
metadata <- subset(metadata, select = -c(Group_cutoff_4))

metadata_names<-metadata
rownames(metadata)<-metadata_names$SampleID
...
for (i in seq_along(featureList)) {
  ...
  # add random binary variable with fitting length
  # if (nrow(premetamat) == 13) {
  #   premetamat$randomBin <- randomBin13
  # } else {
  #   premetamat$randomBin <- randomBin12
  # }
  premetamat$randomBin <- randomBin13[1:nrow(premetamat)]
...
saveRDS(outerList, "/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/input/outerList_circos_plot.rds")
outerList <- readRDS("/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/input/outerList_circos_plot.rds")
#VS_longSafe <- VS_long
...
BQ_names <- data.frame(
  names = c("Glucose (mg/dL)", "Creatinine (mg.dL)",
            "Cholesterol (mg/dL)", "HDL (mg/dL)", "LDL (mg/dL)", 
            "TG (mg/dL)", "Proteins (g/dL)", "ALB (g/dL)",
            "Bilirrubin (mg/dL)", "AST(U/L)", "ALT(U/L)",
            "GGT(U/L)", "FA (mU/mL)", "LEUCOS(x10.9/L)", "HB(g/dL)",
            "Platelets (x10.9/L)", "IQ%", "INR", "Glu","ARG"))
rownames(BQ_names)<-colnames(BQ)

Imm_names <- data.frame(
  names = c("Amonio", "IL6", "IL18","IL23","IL21","IL13","CCL20",
            "TNFa","CXCL13", "IL22","IL4","X5HT",
            "ADMA","CITRU","GABA","GLN", "HIS", "KA", "KYN",
            "SER", "TRP", "XA","Urea"))
rownames(Imm_names)<-colnames(Imm)

SCFAs_names <- data.frame(
  names = c("AA feces", "BA feces", "CA feces", "PA feces", 
            "VA feces", "IBA feces", "IVA feces", "2-MBA feces"))
rownames(SCFAs_names)<-colnames(SCFAs)

QoL_names <- data.frame(
  names = c("Physical funtion", "Physical role", "Emotional role", "Vitality", 
            "Mental health","Social function","Pain","General health",
            "Health Transition", "Final Score SF36","Physical component summary",
            "Mental component summary"))
rownames(QoL_names)<-colnames(QoL)

allNiceNames <- list("Biochemistry" = BQ_names,
...
color_palette <- c("Biochemistry" = "#999999", 
                   "Immunology" = "#E69F00", 
                   "SCFAs" = "#56B4E9", 
                   "QoL" = "#009E73", 
                   "Genera" = "#CC79A7", 
...
data_all <- data_all[data_all$Ps <= 0.0001, ]
...
svg(filename = "circos_plot_40_0001.svg", width = 10, height = 8)
par(mfrow=c(1,2))
circos.clear()
