...
#SBATCH --job-name=rpoB_diamond_MAGs
...
OUTDIR="/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/Assembly/Segatella_copri/results/rpoB_in_MAGs"

mkdir -p "$OUTDIR"

# ------------------ Check rpoB_PROT_FA ------------------
if [ ! -s "$rpoB_PROT_FA" ]; then
    echo "ERROR: rpoB_PROT_FA file is missing or empty"
    exit 1
fi

if ! head -n1 "$rpoB_PROT_FA" | grep -q "^>"; then
    echo "ERROR: rpoB_PROT_FA file does not start with >"
    exit 1
fi
...
if [ ! -s "$MAG" ]; then
    ...
    echo "$MAG_NAME: SKIPPED (empty MAG)" > "${MAG_OUT}/${MAG_NAME}_rpoB_presence.txt"
    exit 0
fi

if ! head -n1 "$MAG" | grep -q "^>"; then
    echo "WARNING: MAG file $MAG is malformed. Skipping."
    echo "$MAG_NAME: SKIPPED (malformed MAG)" > "${MAG_OUT}/${MAG_NAME}_rpoB_presence.txt"
    exit 0
fi
...
# ------------------ DIAMOND blastp (rpoB vs MAG) ------------------
OUTPUT="${MAG_OUT}/${MAG_NAME}_rpoB_hits.tsv"
echo "Searching for rpoB in $MAG_NAME..."
diamond blastp \
    -q "$rpoB_PROT_FA" \
    -d "$DIAMOND_DB" \
    -o "$OUTPUT" \
...
if [ -s "$OUTPUT" ]; then
    echo "$MAG_NAME: rpoB detected" > "$PRESENCE_FILE"
else
    echo "$MAG_NAME: rpoB NOT detected" > "$PRESENCE_FILE"
fi
