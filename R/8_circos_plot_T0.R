...
ps_clr_corrected<- subset_samples(ps_clr_corrected, Timepoint %in% c("T0"))
...
ARGs_ok_corrected<- subset_samples(ARGs_ok_corrected, Timepoint %in% c("T0"))
...
saveRDS(outerList, "/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/input/outerList_circos_plot_T0.rds")
outerList <- readRDS("/fast/AG_Forslund/Lola/Secuencias_INCLIVA_2024/MHE_rif/input/outerList_circos_plot_T0.rds")
#VS_longSafe <- VS_long
...
svg(filename = "circos_plot_40_01_T0.svg", width = 10, height = 8)
par(mfrow=c(1,2))
circos.clear()
