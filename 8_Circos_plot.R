...
ARGs_ok_corrected <- readRDS("~/OneDrive - Universitat de Valencia/Doctorado/Estudios/INCLIVA_2024/MHE_rif/outputs/merged/ARGs_ok_corrected.Rds")
...
ARGs_ok<-as.data.frame(t(otu_table(ARGs_ok_corrected)))
ARGs_ok<-ARGs_ok[, order(colnames(ARGs_ok))]
ARGs_ok<-ARGs_ok[order(rownames(ARGs_ok)),]
...
metadata <- metadata %>% mutate(Timepoint = ifelse(Timepoint=="T0",1,0))
...
GMMs$names<-GMMs$V2
GMMs$V2<-NULL
ARGs_ok<- as.data.frame(tax_table(ARGs_ok_corrected))
ARGs_ok$names<-ARGs_ok$Best_Hit_ARO
ARGs_ok<-ARGs_ok[,c("names", "Best_Hit_ARO")]
ARGs_ok$Best_Hit_ARO<-NULL
...
svg(filename = "circos_plot_40_0001_2.svg", width = 10, height = 8)
par(mfrow=c(1,2))
circos.clear()
